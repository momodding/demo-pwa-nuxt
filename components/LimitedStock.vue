<template>
  <BaseModal
    ...
    :show-header="true"
    :show-footer="true"
    passed-component="LimitedStock"
  >
    <div class="row no-gutters">
      <div class="col-4 col-md-5">
        <img :src="payload.picture" />
      </div>
      <div class="col-7 offset-1 col-md-6 offset-md-1">
        <p>
          <strong>{{ payload.name }}</strong>
        </p>
        <p><strong>SKU :</strong> #{{ payload.sku }}</p>
        <p><strong>Color :</strong> {{ payload.color }}</p>
        <p>
          <strong>Price :</strong>
          {{ parseInt(payload.price) | rupiah }}
        </p>
        <p class="text-danger">
          You cannot purchase more than
          {{ payload.stockLeft }} pcs available
        </p>
      </div>
    </div>
  </BaseModal>
